...
public:
  ...
  void findCenter(int n, const std::vector<geometry_msgs::Point> &pointList, geometry_msgs::Point &center){
    ...
    for (int i = 0; i < pointList.size(); ++i){
      ...
      if(dist > 0.8 && dist < 1.04){
        ...
        cout<<"Angle = "<<center.z<<endl;
      }
      // Find center using diagonal of the table
      if (dist > 1.4 && dist < 1.7){
        center.x = 0.5*cos(center.z)+(temp.x + pointList[i].x)/2;
        center.y = 0.5*sin(center.z)+(temp.y + pointList[i].y)/2; 
      }
    }
  }
  ...
  void scanCallback (const sensor_msgs::LaserScan::ConstPtr& scan_in)
  {
    ...
    for(int i=1; i<n; i++){
      ...
      // If ponint is within a radius of 30 cm then it's the same point 
      if (dist < 0.5){
